...
namespace PuzzleAnimator {
        ...
        const AnimationConfig& config)
    {
        ...
        // Use provided initial positions and rotations
        ...
        if (startPositions.size() != features.size()) {
            cout << "Warning: Initial positions size mismatch, using random positions" << endl;
            startPositions.resize(features.size());
            startRotations.resize(features.size(), 0.0f);
            srand(42);
